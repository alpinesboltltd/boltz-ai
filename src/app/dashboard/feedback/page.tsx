...
export default function FeedbackPage() {
    ...
    async function loadFeedback() {
      try {
        ...
        const mockFeedback = Array.from({ length: 20 }, (_, i) => {
          ...
          return {
            id: `feedback_${i}`,
            chatbotId: mockChatbots[botIndex].id,
            chatbotName: mockChatbots[botIndex].name,
            rating,
            comment: rating >= 4 
  ...
  const filteredFeedback = feedback.filter(item => {
    const matchesChatbot = selectedChatbot === 'all' || item.chatbotId === selectedChatbot;
    const matchesRating = 
      filter === 'all' || 
  ...
  const positivePercentage = feedback.length > 0
    ? Math.round((feedback.filter(item => item.rating >= 4).length / feedback.length) * 100)
    : 0;
  
  const negativePercentage = feedback.length > 0
    ? Math.round((feedback.filter(item => item.rating <= 2).length / feedback.length) * 100)
    : 0;
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Review feedback from users who have interacted with your chatbots.
          </p>
        </div>
      </div>
          ...
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              <div className="flex-shrink-0 bg-green-100 rounded-md p-3">
                <FaceSmileIcon className="h-6 w-6 text-green-600" aria-hidden="true" />
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Positive Feedback</dt>
                  <dd>
                    <div className="text-lg font-medium text-gray-900">{positivePercentage}%</div>
                  </dd>
                </dl>
              </div>
            </div>
          ...
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              <div className="flex-shrink-0 bg-red-100 rounded-md p-3">
                <FaceFrownIcon className="h-6 w-6 text-red-600" aria-hidden="true" />
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Negative Feedback</dt>
                  <dd>
                    <div className="text-lg font-medium text-gray-900">{negativePercentage}%</div>
                  </dd>
                </dl>
              </div>
            </div>
                ...
                <div className="px-4 py-4 sm:px-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <p className="text-sm font-medium text-primary-600">{item.chatbotName}</p>
