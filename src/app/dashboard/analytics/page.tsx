    ...
    >
      <div className="flex items-center justify-center h-full">
        <p className="text-gray-500 text-sm">
          Chart visualization would appear here ({type} chart with {data.length}{" "}
          data points)
        </p>
      </div>
    </div>
  ...
  useEffect(() => {
    async function loadAnalytics() {
      try {
        ...
        // const response = await fetch(`/api/analytics?timeRange=${timeRange}&chatagentId=${selectedChatbot}`);
        ...
        const conversationsByDay = Array.from(
          { length: days },
          () => Math.floor(Math.random() * 100) + 20
        );
        const messagesByDay = conversationsByDay.map(
          (c) => c * (Math.floor(Math.random() * 5) + 3)
        );
        const totalConversations = conversationsByDay.reduce(
          (sum, val) => sum + val,
          0
        );
        ...
        const mockAnalytics = {
          totalConversations,
          totalMessages,
          avgConversationLength: +(totalMessages / totalConversations).toFixed(
            1
          ),
          ...
          conversationsByDay,
          conversationsByHour: Array.from(
            { length: 24 },
            () => Math.floor(Math.random() * 50) + 5
          ),
          messagesByDay,
          userSatisfaction: Array.from(
            { length: days },
            ...
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd>
                    <div className="text-lg font-medium text-gray-900">
                      {analytics.avgResponseTime} seconds
                    </div>
                  </dd>
                </dl>
              </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd>
                    <div className="text-lg font-medium text-gray-900">
                      {analytics.avgConversationLength} messages
                    </div>
                  </dd>
                </dl>
              </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
        <div>
          ...
          <Chart
            ...
            labels={Array.from(
              { length: analytics.conversationsByDay.length },
              (_, i) => `Day ${i + 1}`
            )}
          />
        </div>
