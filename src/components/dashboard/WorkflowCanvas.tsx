...
export function WorkflowCanvas({ nodes, onNodesChange, onNodeSelect, selectedNode }: WorkflowCanvasProps) {
  const [draggedNode, setDraggedNode] = useState<string | null>(null);
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 });

  const nodeTypes = [
    { type: "trigger", label: "Trigger", color: "bg-green-100 border-green-300 text-green-700" },
    { type: "message", label: "Message", color: "bg-blue-100 border-blue-300 text-blue-700" },
    { type: "api_call", label: "API Call", color: "bg-purple-100 border-purple-300 text-purple-700" },
    { type: "condition", label: "Condition", color: "bg-yellow-100 border-yellow-300 text-yellow-700" },
    { type: "delay", label: "Delay", color: "bg-gray-100 border-gray-300 text-gray-700" },
    { type: "email", label: "Email", color: "bg-red-100 border-red-300 text-red-700" }
  ];

  const getNodeColor = (type: string) => {
    return nodeTypes.find(nt => nt.type === type)?.color || "bg-gray-100 border-gray-300 text-gray-700";
  };
  ...
  return (
    <div className="flex h-full">
      ...
      {/* Canvas */}
      <div className="flex-1 relative overflow-auto bg-gray-25" id="workflow-canvas">
          ...
          {nodes.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-500 mb-4">Start by adding a trigger node from the left panel</p>
                <button
