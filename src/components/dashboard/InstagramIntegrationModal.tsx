...
export default function InstagramIntegrationModal({
  ...
  const handleConnectInstagram = async () => {
    ...
    try {
      ...
      // window.location.href = `/api/integrations/instagram/oauth?chatbotId=${chatbotId}`;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // await fetch(`/api/chatbots/${chatbotId}/integrations/instagram/configure`, {
      //   method: 'POST',
      ...
      setError('Failed to configure Instagram integration. Please check your credentials and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Connect Instagram Direct Messages</h3>
        ...
        {step === 1 ? (
          <div>
            <p className="text-sm text-gray-500 mb-6">
              Connect your chatbot to Instagram Direct Messages to engage with customers directly on Instagram.
            </p>
            
            <div className="space-y-4">
              <div className="bg-pink-50 p-4 rounded-md">
                <h4 className="text-sm font-medium text-pink-800">Before you connect</h4>
                <ul className="mt-2 text-xs text-pink-700 list-disc list-inside space-y-1">
                  <li>You need to have a Facebook Page connected to an Instagram Professional account</li>
                  <li>You must be an admin of the Facebook Page</li>
                  <li>Your Facebook app needs Instagram messaging permissions</li>
                </ul>
              </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          <h4 className="text-sm font-medium text-gray-900">How to find your Instagram credentials</h4>
          <ol className="mt-2 text-xs text-gray-500 list-decimal list-inside space-y-1">
            ...
            <li>Get your Instagram Business Account ID from the Facebook Graph API</li>
            <li>Generate a long-lived access token with the instagram_manage_messages permission</li>
          </ol>
          <p className="mt-2 text-xs text-gray-500">
            Need help? <a href="https://developers.facebook.com/docs/instagram-api/getting-started" target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:text-primary-500">Read the Instagram API documentation</a>
          </p>
        </div>
      </div>
    </div>
