...
import { apiFunctionSchema, ApiFunctionFormData } from "@/schemas/actionSchemas";
import { X, Save, Globe, Key, Lock, Code, ToggleLeft, ToggleRight } from "lucide-react";
...
export function ApiFunctionBuilder({ onSave, onCancel, initialFunction }: ApiFunctionBuilderProps) {
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<ApiFunctionFormData>({
  ...
  const ToggleButton = ({ isJson, onToggle }: { isJson: boolean; onToggle: () => void }) => (
    <button
      type="button"
      onClick={onToggle}
    ...
    >
      {isJson ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
      {isJson ? 'JSON' : 'Form'}
    </button>
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          <form id="api-function-form" onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          {/* Basic Information */}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <ToggleButton 
                    ...
                    onToggle={() => setToggles(prev => ({ ...prev, headers: !prev.headers }))}
                  />
                </div>
                {toggles.headers ? (
                  <textarea
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <ToggleButton 
                    ...
                    onToggle={() => setToggles(prev => ({ ...prev, queryParams: !prev.queryParams }))}
                  />
                </div>
                {toggles.queryParams ? (
                  <textarea
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <ToggleButton 
                    ...
                    onToggle={() => setToggles(prev => ({ ...prev, requestBody: !prev.requestBody }))}
                  />
                </div>
                {toggles.requestBody ? (
                  <textarea
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input placeholder="Field name" className="px-3 py-2 border border-gray-300 rounded-lg" />
                    <input placeholder="Field value" className="px-3 py-2 border border-gray-300 rounded-lg" />
                  </div>
                )}
                <p className="text-sm text-gray-500 mt-1">Use {{variable_name}} for dynamic values</p>
              </div>
            )}
          </div>
          ...
          <div className="border rounded-lg p-4">
            <h3 className="text-lg font-medium mb-4">Response & Error Handling</h3>
            
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-medium text-gray-700">Response Mapping</label>
                  <ToggleButton 
                    isJson={toggles.responseMapping} 
                    onToggle={() => setToggles(prev => ({ ...prev, responseMapping: !prev.responseMapping }))}
                  />
                </div>
                {toggles.responseMapping ? (
                  ...
                  <div className="space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <input placeholder="Variable name" className="px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                      <input placeholder="Response path" className="px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                    </div>
                  </div>
                )}
                <p className="text-sm text-gray-500 mt-1">Map response fields to variables</p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  {...register("errorHandling.retries", { valueAsNumber: true })}
                  type="number"
                  min="0"
                  max="5"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              ...
              <div>
                ...
                <input
                  {...register("errorHandling.timeout", { valueAsNumber: true })}
                  type="number"
                  min="1000"
                  max="30000"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
