...
import { DocumentIcon, GlobeAltIcon, QuestionMarkCircleIcon, CloudArrowUpIcon } from '@heroicons/react/24/outline';
...
export default function KnowledgeIntegration({ onSourcesChange }: KnowledgeIntegrationProps) {
  const [sources, setSources] = useState<KnowledgeSource[]>([]);
  const [activeTab, setActiveTab] = useState<'website' | 'document' | 'faq'>('website');
  ...
  const handleFaqSave = () => {
    const validPairs = faqPairs.filter(pair => pair.question.trim() && pair.answer.trim());
              ...
              <div key={source.id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg">
                <div className="flex items-center space-x-3">
                  <div className={`w-2 h-2 rounded-full ${
                    ...
                    source.status === 'error' ? 'bg-red-500' : 'bg-gray-300'
                  }`} />
                  <div>
