...
export default function KnowledgeBaseManager({ chatbotId }: KnowledgeBaseManagerProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // });
      // 
      // const response = await fetch(`/api/chatbots/${chatbotId}/knowledge/documents`, {
      //   method: 'POST',
      //   body: formData
      // });
      // 
  ...
  const handleAddWebsite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // const response = await fetch(`/api/chatbots/${chatbotId}/knowledge/websites`, {
      //   method: 'POST',
      ...
      setWebsites(prev => 
        prev.map(site => 
          site.id === newWebsite.id
            ? { ...site, id: `real-${site.id}`, status: 'ready', pageCount: Math.floor(Math.random() * 20) + 5 }
            : site
        )
      );
    } catch (error) {
  ...
  const handleAddFAQ = async (e: React.FormEvent) => {
    ...
    try {
      // In production, this would call the real API
      // const response = await fetch(`/api/chatbots/${chatbotId}/knowledge/faqs`, {
      //   method: 'POST',
      //   headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteDocument = async (id: string) => {
    try {
      // In production, this would call the real API
      // await fetch(`/api/chatbots/${chatbotId}/knowledge/documents/${id}`, {
      //   method: 'DELETE'
      // });
  ...
  const handleDeleteWebsite = async (id: string) => {
    try {
      // In production, this would call the real API
      // await fetch(`/api/chatbots/${chatbotId}/knowledge/websites/${id}`, {
      //   method: 'DELETE'
      // });
  ...
  const handleDeleteFAQ = async (id: string) => {
    try {
      // In production, this would call the real API
      // await fetch(`/api/chatbots/${chatbotId}/knowledge/faqs/${id}`, {
      //   method: 'DELETE'
      // });
    ...
    <div className="bg-white shadow rounded-lg">
      <div className="px-4 py-5 sm:p-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Train your chatbot with your own data to make it more helpful for your users.
        </p>
        ...
        <div className="mt-6">
          ...
          <div className="mt-6">
            {activeTab === 'documents' && (
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  >
                    {loading ? <Spinner size="sm" color="white" /> : 'Upload Documents'}
                  </button>
                  <input
                    type="file"
                    ref={fileInputRef}
                ...
                {documents.length === 0 ? (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
                    ...
                    <p className="mt-1 text-xs text-gray-500">Upload PDF, DOCX, or TXT files to train your chatbot</p>
                  </div>
                ) : (
                  <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                    <table className="min-w-full divide-y divide-gray-300">
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {documents.map((doc) => (
                          <tr key={doc.id}>
                            <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{doc.name}</td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{doc.type}</td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{formatFileSize(doc.size)}</td>
              ...
              <div>
                <div className="mb-4">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Add website URLs to train your chatbot on web content
                  </p>
                </div>
                ...
                {websites.length === 0 ? (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
                    ...
                    <p className="mt-2 text-sm text-gray-500">No websites added yet</p>
                    <p className="mt-1 text-xs text-gray-500">Add website URLs to train your chatbot on web content</p>
                  </div>
                ) : (
                  <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                    <table className="min-w-full divide-y divide-gray-300">
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {websites.map((site) => (
                          <tr key={site.id}>
                            <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{site.url}</td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{site.pageCount}</td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm">
            ...
            {activeTab === 'faqs' && (
              <div>
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-gray-900 mb-2">Add FAQ</h4>
                  <form onSubmit={handleAddFAQ} className="space-y-3">
                    <div>
                      <label htmlFor="question" className="block text-xs font-medium text-gray-700">Question</label>
                      <input
                        type="text"
                        id="question"
                        value={newQuestion}
                    ...
                    <div>
                      <label htmlFor="answer" className="block text-xs font-medium text-gray-700">Answer</label>
                      <textarea
                        id="answer"
                        value={newAnswer}
                    ...
                    <div>
                      ...
                      >
                        {loading ? <Spinner size="sm" color="white" /> : 'Add FAQ'}
                      </button>
                    </div>
                  </form>
                </div>
                ...
                {faqs.length === 0 ? (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
                    ...
                    <p className="mt-1 text-xs text-gray-500">Add frequently asked questions and answers to train your chatbot</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {faqs.map((faq) => (
                      <div key={faq.id} className="bg-white shadow overflow-hidden sm:rounded-lg border border-gray-200">
                        <div className="px-4 py-5 sm:px-6 flex justify-between items-start">
                          <div>
                            <h3 className="text-sm font-medium text-gray-900">{faq.question}</h3>
                            <p className="mt-1 max-w-2xl text-sm text-gray-500">{faq.answer}</p>
                          </div>
                          <button
