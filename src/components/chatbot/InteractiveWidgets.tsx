...
import { CalendarIcon, CreditCardIcon, ShoppingCartIcon } from '@heroicons/react/24/outline';
...
export function CalendarWidget({ availableTimes, onTimeSelected, onClose }: CalendarWidgetProps) {
...
export function PaymentMethodWidget({ onAddCard, onSelectCard, onClose, savedCards }: PaymentMethodProps) {
  ...
  const paymentProviders = [
    ...
    { id: 'moneypoint', name: 'MoneyPoint', logo: '/images/moneypoint-logo.png' }
  ];
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-sm border border-gray-200 p-4 my-4">
      ...
      {!showAddCard ? (
        <>
          <div className="mb-4">
            <h4 className="text-sm font-medium text-gray-700 mb-2">Saved Cards</h4>
            {savedCards.length > 0 ? (
                ...
                {savedCards.map((card) => (
                  ...
                  >
                    <div className="flex items-center">
                      {card.brand === 'visa' && <span className="text-blue-600 font-bold mr-2">Visa</span>}
                      {card.brand === 'mastercard' && <span className="text-red-600 font-bold mr-2">MC</span>}
                      {card.brand === 'amex' && <span className="text-blue-800 font-bold mr-2">Amex</span>}
                      <span className="text-gray-600">•••• {card.last4}</span>
                    </div>
                    <span className="text-gray-500 text-sm">{card.expMonth}/{card.expYear}</span>
                  </div>
                ))}
              </div>
            ) : (
          ...
          <div className="mb-6">
            <h4 className="text-sm font-medium text-gray-700 mb-2">Payment Providers</h4>
              ...
              {paymentProviders.map((provider) => (
                <div
                  ...
                  className={`flex flex-col items-center justify-center p-3 border rounded-md cursor-pointer hover:bg-gray-50 ${
                    selectedProvider === provider.id ? 'border-primary-500 bg-primary-50' : 'border-gray-200'
                  }`}
                >
...
export function ProductSearchWidget({ products, onAddToCart, onViewDetails, onClose }: ProductSearchProps) {
  return (
    <div className="w-full bg-white rounded-lg shadow-sm border border-gray-200 p-4 my-4">
      <div className="flex justify-between items-center mb-4">
              ...
              <div className="ml-4 flex flex-1 flex-col">
                <div>
                  <div className="flex justify-between">
                    ...
                    <p className="ml-4 text-base font-medium text-gray-900">${product.price.toFixed(2)}</p>
                  </div>
...
export function SubscriptionWidget({ currentPlan, availablePlans, onChangePlan, onClose }: SubscriptionWidgetProps) {
    ...
    <div className="w-full bg-white rounded-lg shadow-sm border border-gray-200 p-4 my-4">
      ...
      <div className="mb-4">
        <p className="text-sm text-gray-500">
          Your current plan: <span className="font-medium text-gray-900">{currentPlan}</span>
        </p>
      </div>
...
export function BankTransferWidget() {
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-sm border border-gray-200 p-4 my-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-medium text-gray-900">Secure Bank Transfer</h3>
      ...
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-md mb-4">
        <p className="text-sm text-yellow-700">
          For security reasons, you'll be redirected to your bank's secure authentication page to complete this transaction.
        </p>
      </div>
