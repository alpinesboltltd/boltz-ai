...
export function SystemPromptEditor({ defaultPrompt, onSave }: SystemPromptEditorProps) {
      ...
      {isEditing ? (
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500">
            The system prompt defines your agent's personality, knowledge, and behavior. Be specific about how you want your agent to respond.
          </p>
        </div>
      ) : (
        <div className="bg-gray-50 rounded-md p-4 border border-gray-200">
          <pre className="whitespace-pre-wrap text-sm text-gray-700 font-mono">
            {prompt || "No system prompt defined. Click 'Edit Prompt' to add one."}
          </pre>
        </div>
      )}
    </div>
