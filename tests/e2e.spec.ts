...
test.describe('Dashboard Flow', () => {
    ...
    test('should navigate to Create Agent page', async ({ page }) => {
        ...
        await expect(page.getByText('Loading workspace...')).not.toBeVisible({ timeout: 10000 });

        // Verify Create Agent page content
        await expect(page.getByText('Create New Agent')).toBeVisible();
        await expect(page.getByPlaceholder('e.g. Customer Support Bot')).toBeVisible();
    });
